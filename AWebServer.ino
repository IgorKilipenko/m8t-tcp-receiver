AWebServer::AWebServer(ATcpServer* telnetServer) : ssid{APSSID}, password{APPSK}, gpsStarted{'0'} ,server{80}, ws{"/ws"}, events{"/events"}, telnetServer{telnetServer} {}
...
void AWebServer::setup() {
	...
	server.on("/api/gnss", HTTP_POST, [&](AsyncWebServerRequest *request) {
		...
		logger.debug("Server has gps atr %s\n)", request->hasArg("gps") ? "true" : "false");
		logger.debug("Argument (0) name : %s, argument gps value : %s\n", request->argName(0).c_str(), request->arg("gps").c_str());
		memcpy(gpsStarted, request->arg(F("gps")).c_str(), sizeof(gpsStarted));
...
void AWebServer::onWsEvent(AsyncWebSocket *server, AsyncWebSocketClient *client, AwsEventType type, void *arg, uint8_t *data, size_t len) {
	if (type == WS_EVT_CONNECT) {
		...
		logger.printf("ws[%s][%u] disconnect: %u\n", server->url(), client->id());
